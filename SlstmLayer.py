...
def _ST_OneHot(op, grad):
	...
	return [tf.zeros(tf.shape(op.inputs[0])), 0, 0, 0]
		...
		for time, (x, x_p) in enumerate(zip(x_seq, x_p_seq)):
			...
			with ops.name_scope("MultinomialSample") as name:
				with g.gradient_override_map({"OneHot": "ST_OneHot", "Multinomial": "ST_Multinomial"}):
					action = tf.one_hot(tf.reshape(tf.multinomial(action, 1), [batch_size]), choose_length, on_value = 1.0, off_value = 0.0, dtype=tf.float32, axis = -1)
			...
			seq_length = tf.reshape(incoming[0].seq_length, [batch_size])
			now_h = tf.reduce_sum(tf.stack(all_h, axis = 1) * action, axis = 1)
			now_h = tf.where(tf.less(time, seq_length), now_h, tf.zeros([batch_size, output_dim[0]]))
			now_c = tf.reduce_sum(tf.stack(all_c, axis = 1) * action, axis = 1)
			now_c = tf.where(tf.less(time, seq_length), now_c, tf.zeros([batch_size, output_dim[0]]))
			now_h_p = tf.reduce_sum(tf.stack(all_h_p, axis = 1) * action, axis = 1)
			now_h_p = tf.where(tf.less(time, seq_length), now_h_p, tf.zeros([batch_size, output_dim[1]]))
			now_c_p = tf.reduce_sum(tf.stack(all_c_p, axis = 1) * action, axis = 1)
			now_c_p = tf.where(tf.less(time, seq_length), now_c_p, tf.zeros([batch_size, output_dim[1]]))
